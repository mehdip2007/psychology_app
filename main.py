...
Four responsibilities:
  ...
  3. /obsidian - sync approved notes from an Obsidian vault into production.
  4. /agent    - answer Persian questions using ONLY the production store.
"""
import hashlib
import json
import logging
...
async def ingest_upload(file: UploadFile = File(...)):
    """Upload a PDF or EPUB. It is parked in staging and queued for human review.
...
def obsidian_sync(req: ObsidianSyncRequest):
    ...
    Notes with ``status: pending`` or ``status: rejected`` are skipped.
    Already-synced notes whose content hash has not changed are also skipped.
    If a note was updated since its last sync, old chunks are replaced.
    """
