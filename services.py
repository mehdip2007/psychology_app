"""Supporting services: PDF/EPUB extraction + OCR, translation, embeddings,
...
def extract_text(file_bytes: bytes, filename: str = "") -> dict:
    ...
    Returns a dict: {"text": str, "method": str}
    """
...
def extract_epub(epub_bytes: bytes) -> dict:
    ...
    Reads every document item (chapters, sections) in spine order,
    strips HTML tags, and joins them into a single text block.
    """
    try:
        import io
        import ebooklib
        ...
        return {"text": "\n\n".join(parts), "method": "epub"}

    except Exception as exc:  # noqa: BLE001
        logger.error("EPUB extraction failed: %s", exc)
        return {"text": "", "method": "epub-failed"}
